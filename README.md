...
### 1. 	HDMI
This HDMI 1.4a standard port supports up to 4K (3840x2160) resolution at 24/30Hz. I tested QHD (2560x1440@60Hz). If MST and Cable NVM are not current, it may show black, not always wake up, or not work. According to Dell, it does not support HDCP, but the kernel output shows `HDCP version: HDCP1.4` and it is a required component for full HDMI 1.4a compliance. Tested with HD 1080p video on free streaming services.
...
- The mDP is a hard one to make work. I did multiple flashes and reboots on one dock to finally get it to go, in different orders for MST, Cable, and Dock. I don't really know what and why, but not every Dock has the mDP working right away. Port function seems quite inconsistent. I have three docks, one I use with mDP, one with HDMI and VGA, and the last with USB-C. On the mentioned dock, all main ports (DP, mPD, and HDMI) now work. The second I didn't test, but the third, mDP, won't work, and I've retried many times. I will investigate.
